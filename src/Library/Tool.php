...
namespace Tonkra\Referral\Library;
...
class Tool
{
    ...
    public static function renderTemplate($msg, $data): array|string
    {
        ...
        foreach ($datas[1] as $value) {
            if (array_key_exists($value, $data)) {
                ...
                $msg = preg_replace('/\{' . preg_quote($value, '/') . '\}/', $data[$value], $msg);
            } else {
    ...
    public static function xcopy(string $source, string $dest, bool $overwrite = false, int $permissions = 0755): ?string
    {
        ...
        if (File::exists($dest)) {
            if ($overwrite) {
                // Secure delete existing directory
                $xDelete = self::xdelete($dest);
                if($xDelete !== null){ return $xDelete;}
            } else {
               return "Destination directory already exists.  Enable overwrite to update.";
            }
        }

        // Create destination directory
        if (!File::makeDirectory($dest, $permissions, true, true)) {
            return "Failed to create directory: {$dest}";
        }

        // Copy directory
        File::copyDirectory($source, $dest);

        // Remove unsafe files and set permissions
        $allFiles = File::allFiles($dest);
        foreach ($allFiles as $file) {
            $filePath = $file->getPathname();

            // Check file safety
            if (!self::isSafeFile($filePath)) {
                File::delete($filePath);
                Log::warning("Deleted unsafe file after copy", ['file' => $filePath]);
                continue;
            }
        ...
        $allDirectories = File::directories($dest, true);
    ...
    /**
     * Secure recursive deletion using Laravel's File facade
     * 
     * @param string $path
     * @return void
     * @throws Exception
     */
    public static function xdelete(string $path): ?string
    {
        if (!File::exists($path)) {
            return null;
        }
        ...
        foreach ($protectedPaths as $protected) {
            $realProtected = realpath($protected);
            if ($realProtected && $realPath && $realPath == $realProtected) {
                return "Cannot delete protected directory: {$path}";
            }
        }
    ...
    /**
     * Validate file safety before copy operations
     * 
    ...
    private static function isSafeFile(string $filePath): bool
    {
        ...
        try {
            ...
            Log::warning("MIME type check failed", ['file' => $filePath]);
        }
