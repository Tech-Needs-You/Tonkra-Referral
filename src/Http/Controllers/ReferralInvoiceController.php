...
use Tonkra\Referral\Services\ReferralPaymentService;
    ...
    class ReferralInvoiceController extends Controller
    {
        ...
        protected $referralPaymentService;

        public function __construct(ReferralPaymentService $referralPaymentService)
        {
        ...
        public function index(): Factory|View|Application
        {
            ...
            return view('referral::admin.Invoices.index', compact('breadcrumbs'));
        }
        ...
        public function showRedemption(ReferralRedemption $redemption): Factory|View|Application
        {

            $breadcrumbs = [
                ['link' => url(config('app.admin_path') . "/dashboard"), 'name' => __('locale.menu.Dashboard')],
                ['link' => url(config('app.admin_path') . "/invoices"), 'name' => __('locale.menu.All Invoices')],
                ['name' => __('locale.labels.redemption')],
            ];

            return view('admin.Invoices.view', compact('breadcrumbs', 'redemption'));
        }       
            ...
            if (is_array($data) && count($data) == 2) {
                switch ($data[0]) {
                    ...
                    case 'sender_id':
                        ...
                        $paymentController->createNotification('senderid', $senderid->sender_id, ReferralUser::find($senderid->user_id)->displayName());
                    ...
                    case 'number':
                        ...
                        $paymentController->createNotification('number', $number->number, ReferralUser::find($number->user_id)->displayName());
                    ...
                    case 'keyword':
                        ...
                        $paymentController->createNotification('keyword', $keyword->keyword_name, ReferralUser::find($keyword->user_id)->displayName());
                    ...
                    case 'subscription':
                        ...
                        if ($user->customer->subscription) {
                            ...
                            // $currentPeriodEndsAt   = $subscription->getPeriodEndsAt($subscription->current_period_ends_at);
														// Updated for better timeframe: to prevent Carbon date error due to continuous renewal of subscription
                        ...
                        if (Helper::app_config('subscription_notification_email')) {
                            ...
                            $admin->notify(new SubscriptionPurchase($invoice));
                        }

                        if ($user->customer->getNotifications()['subscription'] == 'yes') {
                            $user->notify(new SubscriptionPurchase($invoice));
                        }
