 ...
 */
class EloquentReferralAccountRepository extends EloquentReferralBaseRepository implements ReferralAccountRepository
{
    protected ReferralUserRepository $users;
     ...
     */
    public function __construct(ReferralUser $user, ReferralUserRepository $users)
    {
     ...
     */
    public function findOrCreateSocial($provider, $data): ReferralUser
    {
    ...
    public function payPayment(array $input): JsonResponse
    {
        ...
        if ($paymentMethod) {
            ...
            $currency_code = ReferralUser::find(auth()->id())->customer->subscription->plan->currency->code;
            ...
            switch ($paymentMethod->type) {
                ...
                case PaymentMethods::TYPE_PAYPAL:
                    ...
                    $request->body = [
                        'intent'              => 'CAPTURE',
                        'purchase_units'      => [[
                            'reference_id' => ReferralUser::find(auth()->id())->id . '_' . $sms_unit,
                ...
                case PaymentMethods::TYPE_RAZORPAY:
                    ...
                    try {
                        ...
                        if (isset($link->id) && isset($link->short_url)) {
                            ...
                            Session::put('user_id', ReferralUser::find(auth()->id())->id);
                ...
                case PaymentMethods::TYPE_CINETPAY:
                    ...
                    $payment_data = [
                        ...
                        'channels'              => 'ALL',
                        'lang'                  => config('app.locale'),
                        'metadata'              => ReferralUser::find(auth()->id())->id . '-' . $sms_unit . '-' . $price,
                    ];
                ...
                case PaymentMethods::TYPE_0XPROCESSING:
                    ...
                    $payload = [
                        'MerchantId' => $credentials->merchant_id,
                        'ClientId'   => ReferralUser::find(auth()->id())->id,
                        'AmountUsd'  => $price,
                        'Currency'   => $currency_code,
