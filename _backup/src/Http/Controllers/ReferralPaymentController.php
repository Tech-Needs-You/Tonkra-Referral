...
use App\Models\User;
use App\Notifications\AdminTopupNotice;
                    ...
                    if ($plan) {
                        ...
                        if ($invoice) {
                            ...
                            if ($user->sms_unit == null || $user->sms_unit == '-1' || $plan->getOption('sms_max') == '-1') {
                                ...
                                $user->sms_unit = ($plan->getOption('add_previous_balance') == 'yes') ? $user->sms_unit + $plan->getOption('sms_max') : $plan->getOption('sms_max');
                            }
                ...
                if ($request->get('token') == $token) {
                    ...
                    if ($paymentMethod) {
                        ...
                        try {
                            ...
                            if ($response->statusCode == '201' && $response->result->status == 'COMPLETED' && isset($response->id)) {
                                $getSubscriptionData = $this->updateSubscriptionData($plan, $paymentMethod, $response->id);
    ...
    public function offlinePayment($type, Request $request)
    {
        ...
        switch ($type) {
            case 'top_up':
                ...
                $invoice = Invoices::create($invoice_details);

                if ($invoice) {
                    $this->createNotification('topup', 'SMS Unit ' . $sms_unit, $user->displayName());

                    $mail_details = [
                        'amount'         => Tool::format_price($price, $user->customer->subscription->plan->currency->format),
                        'payment_method' => $paymentMethod->name,
                        'type'           => ucwords(Invoices::TYPE_TOPUP),
                        'sms_unit'         => $sms_unit,
                    ...
                    $admin->notify(new AdminTopupNotice($mail_details));
            ...
            case 'subscription':
                ...
                // add log
                $log = $subscription->addLog(SubscriptionLog::TYPE_CLAIMED, [
                    'plan'  => $subscription->plan->getBillableName(),
                    'price' => $subscription->plan->getBillableFormattedPrice(),
                ], $transaction->id);
